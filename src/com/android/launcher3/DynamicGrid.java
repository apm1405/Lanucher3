...
import com.mediatek.common.featureoption.FeatureOption;//add by qinjian
import com.mediatek.common.featureoption.FeatureOption;//add 20140902 for HOSIN_CUST_LAUNCHER_MESSI
...
class DeviceProfile {
	...
	/// HOSIN: HOSIN_CUSTOMIZE_ANDROID_LAUNCHER3 add for search drop bar margin  @{
	int searchDropBarEdgeMarginPx;
	/// HOSIN: HOSIN_CUSTOMIZE_ANDROID_LAUNCHER3 add for search drop bar margin  @}
    ...
    DeviceProfile(Context context,
		...
		/// HOSIN: add for search drop bar margin  @{
		if (com.mediatek.common.featureoption.FeatureOption.HOSIN_CUSTOMIZE_ANDROID_LAUNCHER3) {
			searchDropBarEdgeMarginPx = resources.getDimensionPixelSize(R.dimen.dynamic_grid_edge_margin_for_search_drop_bar);
		}
		// HOSIN: add for search drop bar margin  @}
		...
		/// HOSIN: add for search drop bar 
		if (com.mediatek.common.featureoption.FeatureOption.HOSIN_CUSTOMIZE_ANDROID_LAUNCHER3) {
			searchBarSpaceHeightPx = searchBarHeightPx 
				+ 2 * searchDropBarEdgeMarginPx;
		} else {
        ...
        */
		...
		if(FeatureOption.HOSIN_CUST_LAUNCHER_MESSI){
			 folderCellWidthPx = cellWidthPx + 5 * edgeMarginPx;
        	 folderCellHeightPx = cellHeightPx + (int) ((7f/2f) * edgeMarginPx);
		     folderBackgroundOffset = -edgeMarginPx;
		     folderIconSizePx = iconSizePx + 0 * -folderBackgroundOffset;
		}
		else{
    ...
    void updateFromConfiguration(Resources resources, int wPx, int hPx,
        ...
        if (isLandscape) {
         ...
         //Hosin zhangjuntao update menu icon size in 20140617
	if (FeatureOption.HOSIN_DEFAULT_MENU_ICON_SIZE){
             allAppsNumCols = 4; 
         }else{
    ...
    Rect getWorkspacePadding(int orientation) {
        ...
        if (orientation == CellLayout.LANDSCAPE &&
            ...
            if (isTablet()) {
				...
				/// HOSIN: for launcher3 customization
                padding.set(desiredWorkspaceLeftRightMarginPx - defaultWidgetPadding.left,
                        (com.mediatek.common.featureoption.FeatureOption.HOSIN_CUSTOMIZE_ANDROID_LAUNCHER3)
						? 0
						: searchBarSpaceHeightPx,
    ...
    public void layout(Launcher launcher) {
        ...
        if (hasVerticalBarLayout) {
			...
			/// HOSIN: add for change the position of search bar
			if (com.mediatek.common.featureoption.FeatureOption.HOSIN_CUSTOMIZE_ANDROID_LAUNCHER3) {
				lp.topMargin = -searchBarSpaceHeightPx;
				searchBar.setPadding(
                    2 * searchDropBarEdgeMarginPx,
                    2 * searchDropBarEdgeMarginPx,
                    2 * searchDropBarEdgeMarginPx, 0);
			} else {
				searchBar.setPadding(
                    2 * edgeMarginPx,
                    2 * edgeMarginPx,
...
public class DynamicGrid {
    ...
    public DynamicGrid(Context context, Resources resources,
        ...
        // by qinjian update for celkon_q3000 that menu app ico change a little 2014-05-23
       if(FeatureOption.HOSIN_MAIN_MENU_APP_ICO_CHANGE_SAMLL){
          deviceProfiles.add(new DeviceProfile("Nexus S",
                296, 491.33f,  4, 4,  46, 13, (hasAA ? 5 : 4), 48));
        }else{
          deviceProfiles.add(new DeviceProfile("Nexus S",
                296, 491.33f,  4, 4,  48, 13, (hasAA ? 5 : 4), 48));
        }
       // by qinjian update for celkon_q3000 that menu app ico change a little ---end

        /// M: add 294 X 460 profile
        deviceProfiles.add(new DeviceProfile("294 X 460",
                294, 460,  4, 4,  50.34943f, 13, (hasAA ? 5 : 4), 49.566288f));
		/// HOSIN: add "HOSIN K-II 720P" profile (4 X 5 style, compare with "Nexus S"/"294x460"/"Nexus 4")
		if (com.mediatek.common.featureoption.FeatureOption.HOSIN_CUSTOMIZE_ANDROID_LAUNCHER3) {
			deviceProfiles.add(new DeviceProfile("HOSIN K-II 720P",
					287, 567, 4, 4, 56.1f/*51.63f*/, 13, (hasAA ? 5 : 4), 54/*50.59f*/));
		}
	...
	//Hosin zhangjuntao update menu icon size in 20140617
	if (FeatureOption.HOSIN_DEFAULT_MENU_ICON_SIZE){
             deviceProfiles.add(new DeviceProfile("QMobile",
                287, 567,  4, 4,  48, 12, (hasAA ? 5 : 4), 48)); 
         }
