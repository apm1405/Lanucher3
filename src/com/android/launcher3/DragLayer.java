...
import java.lang.reflect.Method;//add for HOSIN_CUST_LAUNCHER_MESSI
import com.mediatek.launcher3.ext.LauncherLog;
import android.util.Log;//add for HOSIN_CUST_LAUNCHER_MESSI
import java.util.ArrayList;
import com.mediatek.common.featureoption.FeatureOption;////add for HOSIN_CUST_LAUNCHER_MESSI
/**
...
//HOSIN_CUST_LAUNCHER_MESSI
    ...
    private boolean expandBar = false;;
//HOSIN_CUST_LAUNCHER_MESSI
    /**
    ...
    public DragLayer(Context context, AttributeSet attrs) {
        super(context, attrs);
		mContext = context; //add by messi 20140902 for HOSIN_CUST_LAUNCHER_MESSI
	...
	else if(action == MotionEvent.ACTION_MOVE){// add by messi 20140902 forHOSIN_CUST_LAUNCHER_MESSI
	if(FeatureOption.HOSIN_CUST_LAUNCHER_MESSI){
		endY =  (int)ev.getY();
		endX = (int)ev.getX();
		scrollY= endY - startY;
		scrollX= Math.abs(endX - startX);
		if(scrollX>50){
			expandBar = false;
		}
		if(scrollY>150&&expandBar&&!mDragController.dragging()){
			openNotify();
			expandBar = false;
			}
		}
...
//add 20140902 for HOSIN_CUST_LAUNCHER_MESSI end
	...
	//add 20140902 for HOSIN_CUST_LAUNCHER_MESSI messi
        else if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            if (mTouchCompleteListener != null) {
                mTouchCompleteListener.onTouchComplete();
            }
