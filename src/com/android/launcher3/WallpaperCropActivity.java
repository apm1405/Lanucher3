...
public class WallpaperCropActivity extends Activity {
    ...
    protected static class BitmapCropTask extends AsyncTask<Void, Void, Boolean> {
        ...
        public boolean cropBitmap() {
			...
			int len = -1;
			Log.i("messi","mInStream: "+mInStream);
			...
			Log.i("messi","data: "+data.length);
			BitmapFactory.Options opts = new BitmapFactory.Options();
			opts.inJustDecodeBounds = false;
			Bitmap bitmap = BitmapFactory.decodeByteArray(data,0,data.length,opts);
			Log.i("messi","bitmap: "+bitmap);
			int width = Utilities.getScreenWidth(mContext);
			int height = Utilities.getScreenHeight(mContext);
...
;			ByteArrayOutputStream bos = new ByteArrayOutputStream();
			...
			Uri newUrl = mContext.getContentResolver().insert(url,values);
		...
		*/
			...
			String[] array = mContext.getResources().getStringArray(R.array.wallpaper_chooser);//messi0909
			if(FeatureOption.HOSIN_CUST_LAUNCHER_MESSI){
				if(WallpaperPickerActivity.wallpaperType.equalsIgnoreCase(array[0])){
	                           	wallpaperManager.setStream(new ByteArrayInputStream(outByteArray));
				    }
				else if(WallpaperPickerActivity.wallpaperType.equalsIgnoreCase(array[1])){
					Bitmap bitmap = BitmapFactory.decodeStream(new ByteArrayInputStream(outByteArray));
					int width = Utilities.getScreenWidth(mContext);
					int height = Utilities.getScreenHeight(mContext);
					int x = (bitmap.getWidth() -width)/2;
					int y = (bitmap.getHeight() - height)/2;
					...
					values.put("flag",1);
	   			 	  ...
	   			 	  Log.i("messi","insert end: "+newUrl);
		     			if(newUrl == null){
		     				Log.i("messi","update start"+array[1]);
		     				mContext.getContentResolver().update(url,values,"_id =  ?",new String[]{"1"});
		     			}	
                        	}
				else if(WallpaperPickerActivity.wallpaperType.equalsIgnoreCase(array[2])){
					Bitmap bitmap = BitmapFactory.decodeStream(new ByteArrayInputStream(outByteArray));
					int width = Utilities.getScreenWidth(mContext);
					int height = Utilities.getScreenHeight(mContext);
					int x = (bitmap.getWidth() -width)/2;
					...
					bitmapCrop.compress(CompressFormat.JPEG,85,bos);
						...
						values.put("flag",2);
		     			...
		     			if(newUrl == null){
		     				Log.i("messi","update start");
	     					mContext.getContentResolver().update(url,values,"_id = ?",new String[]{"2"});
	     				}
				}
                        }
			else
